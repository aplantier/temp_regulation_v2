...
function init() {
   ...
   /*tempSeries= [{name: 'ESP1', data: []},
		 {name: 'ESP188', data: []},
		 {name: 'ESP3', data: []}];
   tempColo= ['red', 'green', 'blue']; */
   listEsp = getListEsp();
   listColor=[];
   listSeries=[];
   espSerie=[]
   console.log("List esp : ");
   console.log(listEsp);
//   console.log(tempSeries);
  // console.log(espSerie);
...
//1    get_samples('/esp/temp', chart1.series[i], esp);
    //calls a function or evaluates an expression at specified
    //intervals (in milliseconds).
/*2    window.setInterval(get_samples,
		       refreshT,
...
function getListEsp(listEsp){
    node_url = 'http://localhost:3000/esp'
   $.ajax({
   url: node_url, // URL to "GET" : /esp/temp ou /esp/light
        type: 'GET',
	...
	data: {}, // parameter of the GET request
        success: function (resultat, statut) { // Anonymous function on success
           ...
           console.log(element);
               ...
               listEspMac.push(element._id);
           console.log(listEspMac);
		//listeData.push([Date.now(),element.value]);
            });
           listEsp.setData(listEspMac);
           console.log(listEspMac);
        },
