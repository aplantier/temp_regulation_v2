...
// Topics MQTT
const TOPIC_LIGHT = '/sensor/light'
const TOPIC_TEMP  = '/sensor/temp'
...
// ++++++++++++++++++++=  TP 4 +++++++++++++++++++
       // Table containing the esp sending 
   ...
   var options={
	...
	client_mqtt.on('message', function (topic, message) {
	    ...
	    if (index === -1){
         ...
         // ++++++++++++++++++++=  TP 4 +++++++++++++++++++
          // If the curent pusher is not yest registered, we add it to the db 
          //
          dbo.collection("esp").updateOne({_id: wh}, {$set:{_id:wh}}, {upsert: true});
/*          dbo.collection("esp").find({_id: wh},{$upsert: true}).toArray(function(err,res){
	    console.log("Looking for esp ", message.toString());
         if(res.length==0){// doesn't exist 

            var frTime = new Date().toLocaleString("sv-SE", {timeZone: "Europe/Paris"});
       	    var new_esp = { date: frTime, // timestamp the value 
			      _id: wh      // identify ESP who provide 
			    };
	   
          dbo.collection("esp").insertOne(new_esp, function(err, res) {
               if (err) throw err;
               console.log("\nNew esp has been registered  : ", new_esp);
            });
         }
          else{
               console.log("\nNew esp has not be found : ", res);
...
//+++++++++++++++++++++++++++ TP4 +++++++++++++++++++++
// Getting the list of registered esp 
app.get('/esp', function (req, res) {

    console.log(req.originalUrl);
    console.log("Getting List of resitered esp");

   dbo.collection("esp").find().sort({_id:-1}).toArray(function(err, result) {
	if (err) throw err;
	console.log('get on ', "esp");
	console.log(result);
